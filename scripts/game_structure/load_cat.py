...
def json_load():
    ...
    for i, cat in enumerate(cat_data):
        try:
            ...
            new_cat.pelt = Pelt(
                ...
                fur_type=cat["fur_type"] if "fur_type" in cat else None,
                topear=cat["topear"] if "topear" in cat else random.choice([True, False]),
                top=cat["top"] if "top" in cat else None,
                dumbo=cat["dumbo"] if "dumbo" in cat else None,
            ...
            if "ID" in cat:
                ...
                key = f" at index {i} "
            game.switches['error_message'] = f'Rat{key}in clan_cats.json is missing {e}!'
            game.switches['traceback'] = e
            raise
    ...
    for cat in all_cats:
        ...
        try:
            ...
            logger.exception(f'There was an error loading relationships for rat #{cat}.')
            game.switches['error_message'] = f'There was an error loading relationships for rat #{cat}.'
            game.switches['traceback'] = e
            raise
        ...
        try:
            ...
            logger.exception(f'There was an error when thoughts for rat #{cat} are created.')
            game.switches['error_message'] = f'There was an error when thoughts for rat #{cat} are created.'
            game.switches['traceback'] = e
            raise
...
def csv_load(all_cats):
    ...
    if len(cat_data) > 0:
            ...
            if i.strip() != '':
                ...
                for x in range(len(attr)):
                    ...
                    if attr[x] in ['None', 'None ']:
                        ...
                        attr[x] = False
                game.switches[
                    'error_message'] = '1There was an error loading rat # ' + str(
                        attr[0])
                the_pelt = Pelt(
                    colour=attr[2],
                    name = attr[11],
                ...
                game.switches[
                    'error_message'] = '2There was an error loading rat # ' + str(
                    attr[0])
                ...
                game.switches[
                    'error_message'] = '11There was an error loading rat # ' + str(
                    attr[0])
                if len(attr) > 34:
                ...
                game.switches[
                    'error_message'] = '12There was an error loading rat # ' + str(
                    attr[0])
                if len(attr) > 30:
                ...
                game.switches[
                    'error_message'] = '13There was an error loading rat # ' + str(
                    attr[0])
                if len(attr) > 35:
                    the_cat.dead_for = int(attr[35])
                game.switches[
                    'error_message'] = '14There was an error loading rat # ' + str(
                    attr[0])
                if len(attr) > 36 and attr[36] is not None:
                    the_cat.apprentice = attr[36].split(';')
                game.switches[
                    'error_message'] = '15There was an error loading rat # ' + str(
                    attr[0])
                if len(attr) > 37:
        ...
        game.switches[
            'error_message'] = 'There was an error loading this mischief\'s mentors, apprentices, relationships, or sprite info.'
        for inter_cat in all_cats.values():
            # Load the mentors and apprentices after all cats have been loaded
            game.switches[
                'error_message'] = 'There was an error loading this mischief\'s mentors/apprentices. Last rat read was ' + str(
                inter_cat)
            ...
            if not inter_cat.dead:
                game.switches[
                    'error_message'] = 'There was an error loading this mischief\'s relationships. Last rat read was ' + str(
                    inter_cat)
                inter_cat.load_relationship_of_cat()
            game.switches[
                'error_message'] = 'There was an error loading a mischief\'s sprite info. Last rat read was ' + str(
                inter_cat)
        ...
        if not the_cat.dead:
            ...
            for id in all_cats.keys():
                ...
                game.switches[
                    'error_message'] = f'There was an error when relationships for rat #{the_cat} are created.'

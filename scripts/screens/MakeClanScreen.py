...
class MakeClanScreen(Screens):
    ...
    classic_mode_text = "This mode is Mischief Generator at it's most basic. " \
                        "The player will not be expected to manage the minutia of Mischief life. <br><br>" \
                        "Perfect for a relaxing game session or for focusing on storytelling. <br><br>" \
                        "With this mode you are the eye in the sky, watching the Mischief as their story unfolds. "
    ...
    expanded_mode_text = "A more hands-on experience. " \
                         ...
                         "With this mode you'll be making the important Mischief-life decisions."
    ...
    def screen_switches(self):
        ...
        self.menu_warning = pygame_gui.elements.UITextBox(
            'Note: going back to main menu resets the generated rats.',
    ...
    def handle_name_clan_event(self, event):
        if event.ui_element == self.elements["random"]:
            ...
            if not new_name:
                self.elements["error"].set_text("Your Mischief's name cannot be empty")
                self.elements["error"].show()
                return
            if new_name.casefold() in [clan.casefold() for clan in game.switches['clan_list']]:
                self.elements["error"].set_text("A Mischief with that name already exists.")
                self.elements["error"].show()
                return
    ...
    def on_use(self):
        ...
        if self.sub_screen == 'name clan':
            if self.elements["name_entry"].get_text() == "":
                ...
                self.elements["error"].set_text("Mischief names cannot start with a space.")
                self.elements["error"].show()
                self.elements['next_step'].disable()
    ...
    def open_game_mode(self):
        ...
        self.elements['permi_warning'] = pygame_gui.elements.UITextBox(
            "Your Mischief's game mode is permanent and cannot be changed later.",
    ...
    def open_name_clan(self):
        ...
        # Create all the elements.
        self.elements["random"] = UIImageButton(scale(pygame.Rect((420, 1190), (68, 68))), "",
        ...
        self.elements['next_step'].disable()
        self.elements["name_entry"] = pygame_gui.elements.UITextEntryLine(scale(pygame.Rect((500, 1195), (280, 58)))
                                                                          , manager=MANAGER)
        self.elements["name_entry"].set_allowed_characters(
            list("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_- "))
        self.elements["name_entry"].set_text_length_limit(11)
        self.elements["clan"] = pygame_gui.elements.UITextBox("-Mischief",
                                                              scale(pygame.Rect((750, 1200), (200, 50))),
                                                              object_id="#text_box_30_horizcenter_light",
                                                              manager=MANAGER)
        self.elements["reset_name"] = UIImageButton(scale(pygame.Rect((920, 1190), (268, 60))), "",
    ...
    def clan_name_header(self):
        ...
        self.elements["clan_name"] = pygame_gui.elements.UITextBox(self.clan_name + "Mischief",
    ...
    def open_choose_med_cat(self):
        ...
        # Error message, to appear if you can't choose that cat.
        self.elements['error_message'] = pygame_gui.elements.UITextBox(
            "Too young to become a medicine rat",
            scale(pygame.Rect((300, 706), (1000, 110))),
            object_id=get_text_box_theme("#text_box_30_horizcenter_red"),
            visible=False,
            manager=MANAGER)
    ...
    def open_clan_saved_screen(self):
        ...
        self.elements["save_confirm"] = pygame_gui.elements.UITextBox('Your Mischief has been created and saved!',
